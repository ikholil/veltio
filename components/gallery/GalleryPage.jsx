    ...
    <div className="relative">
        ...
        <div className="fixed top-0 w-full h-full bg-gray-400 flex items-center justify-center z-50">
          <div className="relative rounded-md">
            ...
            />
            <button
              className="absolute top-2 shadow-md hover:bg-red-500 hover:text-white transition-colors duration-300 right-2 p-2 bg-white rounded-full "
       ...
       <div className="grid mx-2 xl:mx-0 grid-cols-12 mb-[50px]  gap-4">
            {
               galleryPageImg.map((item) => (
                <div onClick={() => handleImgClick(item)} key={item.id} className={`col-span-6 cursor-pointer md:col-span-4 ${item.id == 2 && 'row-span-2'} ${item.id == 8 && 'row-span-2'} ${item.id == 9 && 'md:col-span-8'}`}>
                <Image className="zoom" src={item.img} width={item.width} height={item.height} alt="gallery photos" />
            </div>
               )) 
            }
        ...
        <div className="flex justify-center mb-10 md:mb-[120px] gap-2">
            <a href="#" className="bg-[#F112A2] w-7 flex justify-center text-white border border-[#F112A2]">1</a>
            <a href="#" className="text-[#F112A2] w-7 flex justify-center border border-[#F112A2]">2</a>
            <a href="#" className="text-[#F112A2] w-7 flex justify-center border border-[#F112A2]"> {">"}</a>
        </div>
      </div>
    </div>
  );

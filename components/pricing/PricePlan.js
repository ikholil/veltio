    ...
    <div>
      ...
      <div className=" max-w-[1296px]  z-10 flex flex-col items-center mx-auto mb-20 md:mb-28">
        ...
        <div className="flex flex-col px-2 xl:px-0 items-stretch justify-center flex-wrap lg:flex-row gap-6">
            {
              pricing.map((item,i) => (
                <div className="bg-white pricing-card w-[295px] md:w-[417px] shadow-lg rounded-md " key={item.title}>
                  <div className="flex flex-col justify-between">
                    ...
                    <h3 className={`text-lg md:text-3xl text-center font-semibold mb-4 md:mb-10 ${i == 1?'text-white':'text-[#222]'}`}>{item.title}</h3>
                    </div>
                   ...
                   <ul className="flex flex-col gap-4 items-center my-4 md:my-8">
                     {
                      item.features.map(feature => (
                        <li className="text-[#222] text-base md:text-lg  " key={feature}>{feature}</li>
                      ))
                     }
                    </ul>
